...
describe("@fly/cache/response", () => {
    ...
    it("sets a Response with headers", async () => {
        ...
        expect(cachedBody).to.eq(body)
        expect(cachedResponse.status).to.eq(404)
        expect(cachedResponse.headers.get("authorization")).to.be.null
        expect(cachedResponse.headers.get("content-type")).to.eq("text/plain; charset=utf-8")
    })
    ...
    it("sets a Response with headers, and custom skip-headers", async () => {
        ...
        expect(cachedResponse.headers.get("authorization")).to.be.null
        expect(cachedResponse.headers.get("content-length")).to.be.null
        expect(cachedResponse.headers.get("content-type")).to.eq("text/plain; charset=utf-8")
        expect(cachedResponse.headers.get("content-encoding")).to.eq("gzip")
    })
